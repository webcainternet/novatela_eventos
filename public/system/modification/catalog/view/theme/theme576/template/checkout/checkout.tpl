...
$(document).delegate('#button-register', 'click', function() {
    ...
    if ($('input:radio[name=custom_field\\[account\\]\\[1\\]]:checked').val() == 1) {
      $("#text-danger-cpf").remove();
      if (validarCPF($("#input-payment-custom-field2").val()) == false) {
        $("#input-payment-custom-field2").val('');
        var element = $("#input-payment-custom-field2");
        $(element).after('<div class="text-danger-editado" id="text-danger-cpf">CPF inválido!</div>');
      }
    }

    //Valida CNPJ
    if ($('input:radio[name=custom_field\\[account\\]\\[1\\]]:checked').val() == 2) {
      $("#text-danger-cnpj").remove();
      if (validarCNPJ($("#input-payment-custom-field9").val()) == false) {
        $("#input-payment-custom-field9").val('');
        var element = $("#input-payment-custom-field9");
        $(element).after('<div class="text-danger-editado" id="text-danger-cnpj">CNPJ inválido!</div>');
      }
    }

    if (validarCEP($("#input-payment-postcode").val()) == false) {
      $("#input-payment-postcode").val('');
      var element = $("#input-payment-postcode");
      $(element).after('<div class="text-danger-editado" id="text-danger-cep">CEP inválido!</div>');
    } else {

      /*
      //Valida Data Nascimento
      if ($('input:radio[name=custom_field\\[account\\]\\[1\\]]:checked').val() == 1) {
        $("#text-danger-data").remove();
        if (fctValidaData($("#input-payment-custom-field6").val()) == false) {
          $("#input-payment-custom-field6").val('');
          var element = $("#input-payment-custom-field6");
          $(element).after('<div class="text-danger-editado" id="text-danger-data">Data de nascimento inválida!</div>');
        }
      } */
