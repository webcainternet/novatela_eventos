<?php echo $header; ?><?php echo $column_left; ?>
				...
				if($ptbr)
				{
					$tab_mod_rastreio = 'Rastreamento';
				}
				else
				{
					$tab_mod_rastreio = 'Tracking';
				}
				
				if($config->get("mod_rastreio_msg_email")) {
...
<div style="float: right; margin-left: 3px;"><a href="<?php echo $sticker; ?>" target="_blank" data-toggle="tooltip" title="<?php echo $button_address; ?>" class="btn btn-success"><i class="fa fa-file-text"></i></a></div>
      ...
      <div class="panel-body">
        <ul class="nav nav-tabs">
          ...
          <?php } ?>
				...
				<li><a href="#tab-mod-rastreio" data-toggle="tab"><?php echo $tab_mod_rastreio; ?></a></li>
				<?php } ?>
        ...
        <div class="tab-content">
			...
			<?php if($shipping_select == '-1' || $shipping_select == $shipping_code[0]){ ?>
				<div class="tab-pane" id="tab-mod-rastreio">
					<div id="mod_rastreio"></div>
					<table class="table table-bordered">
						<thead>
							<tr>
								<td class="left"><b>Código</b></td>
								<td class="left"><b>Status</b></td>
								<td class="left"><b>Data</b></td>
								<td class="left"><b>Ação</b></td>
							</tr>
						</thead>
						<tbody id="mod_rastreio-tbody"></tbody>
					</table>
					<br />
					...
					<form class="form-inline">					
						<div class="form-group">
							<div class="col-sm-8">
								<input id="mod_rastreio-add-value" name="rastreio[]" type="text" placeholder="separe por virgulas" class="form-control" />
							</div>
							<div class="col-sm-4">
								<a id="mod_rastreio-add" class="btn btn-success">Adicionar</a>
							</div>
						</div>
					</form>
				</div>
  ...
  $.ajax({
    ...
    success: function(html) {
      $('#history').after(html);
			...
			<?php if($config->get("mod_rastreio_msg_email")) { ?>
				<?php if($shipping_select == '-1' || $shipping_select == $shipping_code[0]){ ?>
				var mod_rastreio_url = location.href.replace("info", "getrastreio");
				$('#mod_rastreio-tbody').load(mod_rastreio_url);

				$('#mod_rastreio-add').click(function()
				{
					value = $("#mod_rastreio-add-value").val();
					if(value != '')
					{
						$('#mod_rastreio-tbody').load(mod_rastreio_url+'&mod_rastreio-add-value='+value);
					}
					$("#mod_rastreio-add-value").val('');
				});

				$(document).on('click', '.mod_rastreio-remover', function()
				{
					value = $(this).attr("data-value");
					if(value != '')
					{
						$('#mod_rastreio-tbody').load(mod_rastreio_url+'&mod_rastreio-remove-value='+value);
					}
				});
				<?php } ?>
			<?php } ?>
