...
class ControllerSaleOrder extends Controller {
	private $error = array();
			...
			public function generate_labels()
			{
				...
				{
					$selecteds = $this->request->get['selected'];
					$this->load->library('mod_rastreio');
					$Modrastreio = new Modrastreio($this->registry);
					echo $Modrastreio->generate_labels($selecteds);
				}
			}
			...
			public function getrastreio() {
				if(isset($this->request->get["order_id"]) && !empty($this->request->get["order_id"]))
				{
					...
					if(isset($this->request->get["mod_rastreio-add-value"]) && !empty($this->request->get["mod_rastreio-add-value"]))
					{
						$mod_rastreio_add_value = $this->request->get["mod_rastreio-add-value"];
						$data[$this->request->get["order_id"]] = $mod_rastreio_add_value;
						$this->model_sale_order->updateRastreio($data, true);
					}
					
					if(isset($this->request->get["mod_rastreio-remove-value"]) && !empty($this->request->get["mod_rastreio-remove-value"]))
					{
						$mod_rastreio_remove_value = $this->request->get["mod_rastreio-remove-value"];
						$data[$this->request->get["order_id"]] = $mod_rastreio_remove_value;
						$this->model_sale_order->updateRastreio($data, true, true);
					}
					
					$html = '<tr><td colspan="4">Sem Códigos</td></tr>';
				
					$rastreio = $this->model_sale_order->getRastreio($this->request->get["order_id"]);
					if($rastreio)
					{
						$html = '';
						if(!empty($rastreio["rastreio"]))
						{
							$order_status_id = $rastreio["order_status_id"];

							if(strlen($rastreio["rastreio"]) >= 13 && strstr($rastreio["rastreio"], "{"))
							{
								$this->load->library('mod_rastreio');
								$allrastreio = @unserialize($rastreio["rastreio"]);
								foreach($allrastreio as $codigo => $rastreios)
								{
									$status_text = '';
									$date_added_text = '';
									$html .= '<tr>
									...
									$status_text .= '<ol>';
									$date_added_text .= '<ol>';
									foreach($rastreios as $r)
									{
										$status_text .= '<li>'.$r["status"].'</li>';
										$date_added_text .= '<li>'.date("d/m/Y H:i:s", strtotime($r["date_added"])).'</li>';
									}
									$status_text .= '</ol>';
									$date_added_text .= '</ol>';
									
									// dias postados
									$date_added_text .= '<br />'.count_time_posted($rastreios, false);
									
									$html .= '
									<td>'.$status_text.'</td>
									<td>'.$date_added_text.'</td>';

									if($order_status_id != $this->config->get("mod_rastreio_order_status_final") && !stristr($status_text, "entrega efetuada"))
									{
										$html .= '<td><a onclick="javascript:;" class="btn btn-danger mod_rastreio-remover" data-value="'.$codigo.'">Remover</td>';
									}
									else
									{
										$html .= '<td></td>';
							...
							{
								$allrastreio = explode(",", $rastreio["rastreio"]);
								foreach($allrastreio as $codigo)
								{
									$html .= '<tr>
									<td>'.$codigo.'</td>
									<td>-</td>
									...
									if($order_status_id != $this->config->get("mod_rastreio_order_status_final"))
									{
										$html .= '<td><a onclick="javascript:;" class="btn btn-danger mod_rastreio-remover" data-value="'.$codigo.'">Remover</td>';
									}
									else
	...
	public function index() {
			...
			$this->load->library('mod_rastreio');
			$this->document->addScript('view/javascript/jquery/print.js');
 ...
 $data['button_address'] = $this->language->get('button_address'); 
 ...
 $data['button_address'] = $this->language->get('button_address'); 
  ...
  $data['sticker'] = $this->url->link('sale/order/sticker', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL'); 
			...
			$data['payment_postcode'] = $order_info['payment_postcode'];
			$data['shipping_postcode'] = $order_info['shipping_postcode'];
			$data['rastreio'] = $order_info['rastreio'];
			$data['shipping_code'] = $order_info['shipping_code'];
			$data['config'] = $this->config;
