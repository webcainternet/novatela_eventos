	...
	public function bulkAddCoupon($sample) {
		...
		foreach ($sample as $key => $data) {
	        ...
	        if (empty($coupon_info)) {
		        $this->db->query("INSERT INTO " . DB_PREFIX . "coupon SET name = '" . $this->db->escape($data['name']) . "', code = '" . $this->db->escape($data['code']) . "', discount = '" . (float)$data['discount'] . "', type = '" . $this->db->escape($data['type']) . "', total = '" . (float)$data['total'] . "', logged = '" . (int)$data['logged'] . "',`customer_group_id`  = '" . $this->db->escape(serialize($data['customer_group_id'])) . "' , shipping = '" . (int)$data['free-shipping'] . "', date_start = '" . $this->db->escape(date('Y-m-d', strtotime(str_replace('/', '-', $data['date_start'])))) . "', date_end = '" . $this->db->escape(date('Y-m-d', strtotime(str_replace('/', '-', $data['date_end'])))) . "', uses_total = '" . (int)$data['uses_total'] . "', uses_customer = '" . (int)$data['uses_customer'] . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
