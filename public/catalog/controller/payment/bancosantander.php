...
class ControllerPaymentBancoSantander extends Controller {
	...
	public function index() {
		...
		$data['url_boleto'] = $this->url->link('payment/bancosantander/gerarboleto&pedido='.$this->session->data['order_id'].'');

		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/bancosantander.tpl')) {
			$template = $this->config->get('config_template') . '/template/payment/bancosantander.tpl';
		} else {
			$template = 'default/template/payment/bancosantander.tpl';
		}
	...
	public function confirm() {
		...
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('bancosantander_order_status_id'), $msg, true);
	}
}
?>
